...
public sealed class PartnerProvisioningClient : IPartnerProvisioningClient
{
    private const string InternalApiKeyHeader = "X-Internal-Api-Key";

    private readonly HttpClient _httpClient;
    private readonly PartnerServiceOptions _partnerServiceOptions;

    public PartnerProvisioningClient(
        HttpClient httpClient,
        IOptions<PartnerServiceOptions> partnerServiceOptions)
    {
        _httpClient = httpClient;
        _partnerServiceOptions = partnerServiceOptions.Value;
    }

    public async Task ProvisionPartnerAsync(
        ProvisionPartnerProfileRequest request,
        CancellationToken cancellationToken = default)
    {
        using var message = new HttpRequestMessage(HttpMethod.Post, "/internal/partners/provision")
        {
            Content = JsonContent.Create(request)
        };

        message.Headers.Add(InternalApiKeyHeader, _partnerServiceOptions.InternalApiKey);
