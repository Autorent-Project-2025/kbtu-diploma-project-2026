...
public sealed class WaiveComplaintChargeCommandHandler
{
    ...
    public WaiveComplaintChargeCommandHandler(
    ...
    public async Task<WaiveComplaintChargeResult> Handle(
        WaiveComplaintChargeCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        if (targetCharge.Status.Equals("Paid", StringComparison.OrdinalIgnoreCase))
            throw new ValidationException("Paid charges cannot be waived in this phase. Use refund flow instead.");

        if (targetCharge.Status.Equals("Canceled", StringComparison.OrdinalIgnoreCase))
            throw new ValidationException("Charge is already canceled.");

        // Cancel the pending charge via payment service
        var waived = await _paymentClient.CancelBookingChargeAsync(command.ChargeId, command.Reason, cancellationToken);
        if (!waived)
            throw new InvalidOperationException($"Failed to cancel charge {command.ChargeId}.");
        ...
        try
        {
            ...
            if (conversationId is not null)
            {
                await _chatServiceClient.SendSystemMessageAsync(
                    conversationId,
                    $"Менеджер аннулировал начисление #{command.ChargeId} ({targetCharge.ChargeType}, {targetCharge.Amount:N0} KZT). Причина: {command.Reason.Trim()}",
        ...
        return new WaiveComplaintChargeResult(complaint.ToDto());
    }
}

public sealed record WaiveComplaintChargeResult(ComplaintDto Complaint);
