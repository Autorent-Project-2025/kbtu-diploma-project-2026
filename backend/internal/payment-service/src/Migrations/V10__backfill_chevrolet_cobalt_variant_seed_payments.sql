CREATE TEMP TABLE tmp_cobalt_seed_customer_payments ON COMMIT DROP AS
WITH seed_users AS (
    SELECT *
    FROM (VALUES
...
seed_cars AS (
    SELECT *
    FROM (VALUES
        (1, 11, '22222222-2222-2222-2222-222222222222'::uuid, 596.40::numeric(18, 2), 12),
        (2, 12, '77777777-7777-7777-7777-777777777777'::uuid, 593.60::numeric(18, 2), 10),
        (3, 13, '88888888-8888-8888-8888-888888888888'::uuid, 593.60::numeric(18, 2), 8)
    ) AS seed(sort_order, partner_car_id, partner_user_id, price_hour, billable_hours)
),
...
FROM tmp_cobalt_seed_customer_payments payment
GROUP BY payment.partner_user_id, payment.currency
ON CONFLICT (partner_user_id) DO NOTHING;

CREATE TEMP TABLE tmp_inserted_cobalt_customer_payments ON COMMIT DROP AS
WITH payments_to_insert AS (
    ...
    FROM tmp_cobalt_seed_customer_payments seed
...
FROM (
    ...
    FROM tmp_inserted_cobalt_customer_payments payment
