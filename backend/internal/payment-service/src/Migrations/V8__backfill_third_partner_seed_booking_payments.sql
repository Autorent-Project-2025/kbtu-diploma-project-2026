CREATE TEMP TABLE tmp_third_seed_customer_payments ON COMMIT DROP AS
WITH seed_users AS (
    SELECT *
    FROM (VALUES
...
seed_cars AS (
    SELECT *
    FROM (VALUES
        (1, 8, 1689.50::numeric(18, 2), 10),
        (2, 10, 2409.00::numeric(18, 2), 8),
        (3, 9, 559.00::numeric(18, 2), 12)
...
scheduled_bookings AS (
    SELECT
        ...
        user_seed.user_id,
        '88888888-8888-8888-8888-888888888888'::uuid AS partner_user_id,
...
FROM tmp_third_seed_customer_payments payment
GROUP BY payment.partner_user_id, payment.currency
ON CONFLICT (partner_user_id) DO NOTHING;

CREATE TEMP TABLE tmp_inserted_third_customer_payments ON COMMIT DROP AS
WITH payments_to_insert AS (
    ...
    FROM tmp_third_seed_customer_payments seed
...
FROM (
    ...
    FROM tmp_inserted_third_customer_payments payment
