-- Backfills partner wallet and ledger history for the demo completed bookings
-- seeded in booking-service migration V11__seed_completed_client_bookings.sql.
-- Only missing customer_payments are inserted, so the migration is safe to rerun.

CREATE TEMP TABLE tmp_seed_completed_customer_payments ON COMMIT DROP AS
WITH seed_users AS (
    SELECT *
    FROM (VALUES
...
seed_cars AS (
    SELECT *
    FROM (VALUES
        (1, 1, 3900.00::numeric(18, 2), 8),
        (2, 2, 4700.00::numeric(18, 2), 6),
        (3, 3, 5200.00::numeric(18, 2), 10)
...
FROM tmp_seed_completed_customer_payments payment
GROUP BY payment.partner_user_id, payment.currency
ON CONFLICT (partner_user_id) DO NOTHING;

CREATE TEMP TABLE tmp_inserted_customer_payments ON COMMIT DROP AS
WITH payments_to_insert AS (
    ...
    FROM tmp_seed_completed_customer_payments seed
...
FROM (
    ...
    FROM tmp_inserted_customer_payments payment
