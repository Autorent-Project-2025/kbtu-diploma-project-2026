...
seed_reviews AS (
    SELECT
        22000 + (user_seed.sort_order * 10) + car_seed.sort_order AS comment_id,
        ...
        CASE
            WHEN car_seed.sort_order = 1 AND user_seed.sort_order = 1 THEN 5
            WHEN car_seed.sort_order = 1 AND user_seed.sort_order = 2 THEN 4
            WHEN car_seed.sort_order = 1 THEN 5
            WHEN car_seed.sort_order = 2 AND user_seed.sort_order = 3 THEN 5
            WHEN car_seed.sort_order = 2 THEN 4
            WHEN car_seed.sort_order = 3 AND user_seed.sort_order = 3 THEN 4
            WHEN car_seed.sort_order = 3 THEN 5
            WHEN car_seed.sort_order = 4 AND user_seed.sort_order = 2 THEN 4
            ELSE 5
        END AS rating,
        CASE
            WHEN car_seed.sort_order = 1 AND user_seed.sort_order = 1 THEN 'Premium-feeling cabin and a very calm ride, ideal for a long city day.'
            WHEN car_seed.sort_order = 1 AND user_seed.sort_order = 2 THEN 'The first K5 was clean and comfortable, and pickup took only a few minutes.'
            WHEN car_seed.sort_order = 1 THEN 'Smooth highway behavior, good multimedia, and enough room for passengers.'
            WHEN car_seed.sort_order = 2 AND user_seed.sort_order = 1 THEN 'Second K5 was in good condition, even better in person than on the page.'
            WHEN car_seed.sort_order = 2 AND user_seed.sort_order = 2 THEN 'Nice daily driver with predictable handling and a fresh interior.'
            WHEN car_seed.sort_order = 2 THEN 'Comfortable suspension and no issues during the whole booking.'
            WHEN car_seed.sort_order = 3 AND user_seed.sort_order = 1 THEN 'A rare and emotional car, steering feel was the highlight of the trip.'
            WHEN car_seed.sort_order = 3 AND user_seed.sort_order = 2 THEN 'Very memorable drive and the manual gearbox made it much more engaging.'
            WHEN car_seed.sort_order = 3 THEN 'Fun weekend car, though it demands more attention than a regular sedan.'
            WHEN car_seed.sort_order = 4 AND user_seed.sort_order = 1 THEN 'Strong pull, iconic design, and surprisingly comfortable for a coupe.'
            WHEN car_seed.sort_order = 4 AND user_seed.sort_order = 2 THEN 'The Supra delivered exactly what I expected, great sound and confident acceleration.'
            ELSE 'Memorable trip and a very special car overall, I would book it again.'
        END AS content,
        (
